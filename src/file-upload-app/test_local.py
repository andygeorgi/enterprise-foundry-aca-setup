...
# Configuration
# Flask API sidecar (health + REST endpoints)
        ...
        for file_result in result['results']:
            if file_result.get('processed') and file_result.get('json_file'):
                ...
                analysis_response = requests.get(
                    f"{BASE_URL}/analysis/{file_result['json_file']}")
                if analysis_response.status_code == 200:
                    ...
                    print(
                        f"     - Key-Value Pairs: {len(analysis.get('key_value_pairs', []))}")
                    print(
                        f"     - Content length: {len(analysis.get('content', ''))} characters")
...
def main():
    ...
    print("\nMake sure the app is running: streamlit run app.py --server.port 8080")
    print(f"Flask API sidecar runs on port 8081")
    print("=" * 60)
